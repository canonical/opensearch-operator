...
class OpenSearchPeerClusterRequirer(OpenSearchPeerClusterRelation):
    ...
    def _set_security_conf(self, data: PeerClusterRelData) -> None:
        ...
        # set admin secrets
        admin_pw_field = self.charm.secrets.password_key("admin")
        admin_hash_field = self.charm.secrets.hash_key("admin")
        self.charm.secrets.put(Scope.APP, admin_pw_field, data.credentials.admin_password)
        self.charm.secrets.put(Scope.APP, admin_hash_field, data.credentials.admin_password_hash)
