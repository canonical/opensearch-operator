...
from charms.opensearch.v0.helper_security import generate_password, store_cert, remove_ca, read_ca, \
    store_ca, get_cert_issuer, get_cert_issuer_from_keystore
...
class OpenSearchTLS(Object):
    ...
    def __init__(
        self, charm: "OpenSearchBaseCharm", peer_relation: str, certs_path: str
    ):
    ...
    def read_stored_ca(self, alias: str = CA_ALIAS) -> Optional[str]:
        """Load stored CA cert."""
        if not (secrets := self.charm.secrets.get_object(Scope.APP, CertType.APP_ADMIN.val, peek=True)):
            return None
    ...
    def all_tls_resources_stored(self, only_unit_resources: bool = False) -> bool:  # noqa: C901
        ...
        if not (ca_issuer := get_cert_issuer(cert=current_ca)):
            return False
        ...
        for cert_type in cert_types:
            ...
            cert_issuer = get_cert_issuer_from_keystore(
