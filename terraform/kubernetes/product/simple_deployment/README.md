...
| `juju_integration.traefik_k8s-tls-integration` | [Juju integration](https://registry.terraform.io/providers/juju/juju/latest/docs/resources/integration) | Relates the bundled traefik-k8s to the same TLS provider as OpenSearch. Not created when `ingress_integration` is set. |
...
| `opensearch-dashboards` | Optional OpenSearch Dashboards app definition. traefik-k8s is deployed with it, serving HTTPS, unless `ingress_integration` is set. `tls = true` also encrypts traffic from traefik-k8s to OpenSearch Dashboards. | <pre>object({<br/>  app_name    = optional(string, "opensearch-dashboards-k8s")<br/>  config      = optional(map(string), {})<br/>  channel     = optional(string, "2/edge")<br/>  base        = optional(string, "ubuntu@24.04")<br/>  revision    = optional(number)<br/>  units       = optional(number, 1)<br/>  constraints = optional(string, "arch=amd64")<br/>  resources   = optional(map(string), {})<br/>  tls         = optional(bool, false)<br/>  expose = optional(list(object({<br/>    cidrs     = optional(string)<br/>    endpoints = optional(string)<br/>    spaces    = optional(string)<br/>  })), [])<br/>})</pre> | `null` | no |
...
| `ingress_integration` | Optional external ingress provider for OpenSearch Dashboards, used instead of the bundled traefik-k8s. Ignored when OpenSearch Dashboards is not deployed. Use kind = "endpoint" with name/endpoint for integrations in the same model. Use kind = "offer" with url for cross-model integrations. | <pre>object({<br/>  kind       = string<br/>  name       = optional(string)<br/>  endpoint   = optional(string)<br/>  url        = optional(string)<br/>})</pre> | `null` | no |
...
| `grafana_dashboard_integration` | Optional COS Grafana dashboard provider, related to OpenSearch and OpenSearch Dashboards. Use kind = "endpoint" with name/endpoint for integrations in the same model. Use kind = "offer" with url for cross-model integrations. | <pre>object({<br/>  kind       = string<br/>  name       = optional(string)<br/>  endpoint   = optional(string)<br/>  url        = optional(string)<br/>})</pre> | `null` | no |
| `logging_integration` | Optional COS logging provider, related to OpenSearch and OpenSearch Dashboards. Use kind = "endpoint" with name/endpoint for integrations in the same model. Use kind = "offer" with url for cross-model integrations. | <pre>object({<br/>  kind       = string<br/>  name       = optional(string)<br/>  endpoint   = optional(string)<br/>  url        = optional(string)<br/>})</pre> | `null` | no |
| `metrics_endpoint_integration` | Optional COS metrics scraper, related to OpenSearch and OpenSearch Dashboards. Use kind = "endpoint" with name/endpoint for integrations in the same model. Use kind = "offer" with url for cross-model integrations. | <pre>object({<br/>  kind       = string<br/>  name       = optional(string)<br/>  endpoint   = optional(string)<br/>  url        = optional(string)<br/>})</pre> | `null` | no |
