...
This is a guide on how to restore a backup that was made from a different cluster,
...
To restore your current cluster to the state of the previous cluster,
run the restore command and pass the correct backup-id (from the previously returned list)
to the command:
...
will provide a view if the charm is still running the restore action or not.
