...
This guide shows how to remove nodes from a Charmed OpenSearch cluster without data loss or
...
New units automatically join the cluster and start receiving shard allocations. No additional
...
Run `juju status` and make sure the application is `active` before proceeding. A `maintenance` or
`waiting` status does not necessarily mean something is wrong, but it does indicate that now is not
the best time to scale down — wait for the cluster to settle.

For example, the following output shows an application blocked because replica shards cannot be
assigned:
...
[Cluster health and scaling](explanation-cluster-health). The OpenSearch health API is the source of
truth, so use the check below to confirm.
...
The `get-password` action returns the admin password and the CA certificate chain. Save the CA
certificate to a file (e.g. `cert.pem`) to use with `curl`.

(scale-cluster-health-status)=

### Interpret cluster health

The cluster health API returns `green`, `yellow`, or `red`. For a detailed explanation of what each
state means and how it maps to Juju status, see
...
If it does,
[re-route](https://opensearch.org/docs/latest/api-reference/cluster-api/cluster-reroute/) the shard
...
affected shard to another node. The most common fix is to scale up to restore green health before
scaling down:
...
Do not add units before you know why. Adding capacity only helps for some causes, and an empty node
cannot recreate data that no longer exists. Diagnose the cause first:
...
After removal, OpenSearch reallocates the departed node's shards across the remaining nodes. If the
removed node was `cluster_manager`-eligible, the charm also updates the cluster's voting
...
- [Optimize cluster performance with profiles](how-to-optimize-cluster-performance) — tune resource
  allocation for the new cluster size.
