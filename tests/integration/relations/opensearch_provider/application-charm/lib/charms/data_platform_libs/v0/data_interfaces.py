...
class CachedSecret:
    ...
    def get_info(self) -> Optional[SecretInfo]:
        """Apply the corresponding call on the Secret object within CachedSecret if any."""
        if self.meta:
...
class DataRelation(Object, ABC):
    ...
    def _fetch_my_specific_relation_data(
        ...
        """Fetch data available, (in)directly i.e. secrets, from the rel for owner/this_app."""
        raise NotImplementedError
    ...
    def _fetch_relation_data_with_secrets(
            ...
            if fields:
                ...
                for group in secret_fieldnames_grouped:
                    if contents := self._retrieve_group_secret_contents(
                        ...
                        # If it wasn't found as a secret, give it a 2nd chance as a "normal" field
        ...
        # Processing "normal" fields. May include leftover from what we couldn't retrieve as secret
...
class DataProvides(DataRelation):
    ...
    def _fetch_specific_relation_data(
        ...
        All secret fields are in the Provides side of the databag, so we don't need to worry
        about that
        """
        if not relation.app:
            return {}
