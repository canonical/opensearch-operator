...
async def test_backup_and_restore_under_load(
    ...
    await run_action(ops_test, 0, "create-backup", app=APP_NAME)
    o = ""
    for i in range(0, 10):
        await asyncio.sleep(20)
        o = await run_action(ops_test, 0, "get-backup-status", app=APP_NAME)
        if "completed" in o:
            break
    assert "completed" in o

    backup_id = json.load(await run_action(ops_test, 0, "get-backup-status", app=APP_NAME))[0]
    # Now recover
    await run_action(ops_test, 0, "restore", params={"backup-id": backup_id})
