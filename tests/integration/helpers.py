...
async def get_reachable_unit_ips(ops_test: OpsTest, app: str = APP_NAME) -> List[str]:
    ...
    for ip in await get_application_unit_ips(ops_test, app):
        if not is_reachable(ip, OPENSEARCH_PORT):
            continue
...
async def get_reachable_units(ops_test: OpsTest, app: str = APP_NAME) -> Dict[int, str]:
    ...
    result = {}
    for unit in await get_application_units(ops_test, app):
        if not is_reachable(unit.ip, OPENSEARCH_PORT):
            continue
...
def opensearch_client(
    ...
    return OpenSearch(
        hosts=[{"host": ip, "port": OPENSEARCH_PORT} for ip in hosts],
...
async def cluster_health(
    ...
    if wait_for_green_first:
        try:
            return await http_request(
                ops_test,
                "GET",
                f"https://{unit_ip}:{OPENSEARCH_PORT}/_cluster/health?wait_for_status=green&timeout=1m",
            )
    ...
    return await http_request(
        ops_test,
        "GET",
        f"https://{unit_ip}:{OPENSEARCH_PORT}/_cluster/health",
    )
...
async def check_cluster_formation_successful(
    ...
    """
    response = await http_request(ops_test, "GET", f"https://{unit_ip}:{OPENSEARCH_PORT}/_nodes")
...
async def is_up(ops_test: OpsTest, unit_ip: str, retries: int = 25) -> bool:
    ...
    try:
        for attempt in Retrying(stop=stop_after_attempt(retries), wait=wait_fixed(wait=15)):
            with attempt:
                await http_request(ops_test, "GET", f"https://{unit_ip}:{OPENSEARCH_PORT}/")
                return True
    except RetryError:
        return False
