...
def test_on_s3_broken_steps(
    ...
    with patch(
        "charms.opensearch.v0.opensearch_backups.OpenSearchBackup._plugin_class",
...
class TestBackups(unittest.TestCase):
    ...
    def test_gcs_update_relation_data(self, __, mock_apply_config, _, mock_status) -> None:
        ...
        self.harness.update_relation_data(
            ...
            "s3-integrator",
            {
                ...
                "access-key": "ignore",
                "secret-key": "ewogICJhIjogImIiLAp9Cg==",
                "service-account": "ewogICJhIjogImIiLAp9Cg==",
                "path": TEST_BASE_PATH,
    ...
    @patch("charms.opensearch.v0.opensearch_plugin_manager.OpenSearchPluginManager.apply_config")
    @patch("charms.opensearch.v0.opensearch_distro.OpenSearchDistribution.version")
    def test_s3_update_relation_data(self, __, mock_apply_config, _, mock_status) -> None:
        """Tests if new relation without data returns."""
        mock_status.return_value = PluginState.INSTALLED
        self.harness.update_relation_data(
            self.s3_rel_id,
    ...
    def test_relation_broken(
        ...
        endpoint_type = "charms.opensearch.v0.opensearch_backups.OpenSearchBackup.endpoint_type"
        with patch(endpoint_type, new_callable=PropertyMock) as mock_endpoint:
            mock_endpoint.return_value = "s3"
    ...
    @patch("charms.opensearch.v0.opensearch_distro.OpenSearchDistribution.request")
    @patch("charms.opensearch.v0.opensearch_plugin_manager.OpenSearchPluginManager.status")
    def test_s3_apply_api_config_if_needed(self, mock_status, _, mock_request) -> None:
        """Tests the application of post-restart steps."""
        self.harness.update_relation_data(
            self.s3_rel_id,
            "s3-integrator",
